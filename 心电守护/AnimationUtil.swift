...
class AnimationUtil{
    ...
    static func getImagedataAnimation(duration:CFTimeInterval = 1) -> CAAnimation{
        ...
        scaleAnimation.values = [0.2,0.5, 1, 0.5,0.2]
        scaleAnimation.duration = duration
        ...
        let positionAnimation = CAKeyframeAnimation(keyPath: "position")
        let path = CGMutablePath()
        path.move(to: CGPoint.init(x: screenSize.width/2+80, y: screenSize.height/2))
        path.addLine(to: CGPoint.init(x: screenSize.width/2+40, y: screenSize.height/2-25))
        path.addLine(to: CGPoint.init(x: screenSize.width/2, y: screenSize.height/2-35))
        path.addLine(to: CGPoint.init(x: screenSize.width/2-40, y: screenSize.height/2-25))
        path.addLine(to: CGPoint.init(x: screenSize.width/2-80, y: screenSize.height/2))
        path.move(to: CGPoint.init(x: screenSize.width/2+80, y: screenSize.height/2))
        positionAnimation.path = path
        positionAnimation.duration = duration
