...
class FindpwdViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(modifyPwd(notification:)), name: Notification.Name("modifyPwd"), object: nil)
    ...
    @IBAction func GetBtnAction(_ sender: Any) {
        if PhoneTextField.text?.characters.count == 11{
            netutil.requestVerifyCode(phone: PhoneTextField.text!, type: "2")
        ...
        if phone?.characters.count == 11 {
            if code?.characters.count == 6{
                if (pwd?.characters.count)! >= 6 && (conpwd?.characters.count)! >= 6 {
                    if pwd == conpwd {
                        ToastView.instance.showLoadingView(content: "请稍等...")
                        netutil.modifyPwd(code: code!, pwd: pwd!, phone: phone!)
                    }else{
    ...
    //修改手机密码的通知
    func modifyPwd(notification:Notification) {
        let json:NSDictionary =  notification.object as! NSDictionary
        ToastView.instance.clear()
        let result = json["result"] as! Int
        ...
        print(result)
        if result == 0{
            ToastView.instance.showToast(content: "密码修改成功")
